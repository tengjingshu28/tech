...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-object-2-3.jpg)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-3.png)

其中`sdshdr`是头部, `buf`是真实存储用户数据的地方. 另外注意, 从命名上能看出来, 这个数据结构除了能存储二进制数据, 显然是用于设计作为字符串使用的, 所以在buf中, 用户数据后总跟着一个\0. 即图中 `"数据" + "\0"`是为所谓的buf。

- 如下是**6.0源码中sds相关的结构**：

![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-1.png)

通过上图我们可以看到，SDS有五种不同的头部. 其中sdshdr5实际并未使用到. 所以实际上有四种不同的头部, 分别如下:

![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-2.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/redis-ds-2.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-5.png)

整个ziplist在内存中的存储格式如下：

![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-6.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-7.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-4.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-13.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-8.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-9.png)

如果我们增加如下两级索引，那么它搜索次数就变成了3次

![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-10.png)
...
![img](/Database/Redis/images/进阶-数据结构-底层数据结构详解/db-redis-ds-x-11.png)
