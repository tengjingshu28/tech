...
![](images/6_Map_HashMap&HashSet/HashMap_1.png)
...
![](images/6_Map_HashMap&HashSet/HashMap_2.png)
...
hash(key)的二进制为11100100。

 table.length = 16，table.length - 1 = 15 = 1111(二进制)

11100100 & 1111 = 0100 = 4 (十进制)，相当于取hash(key)的低四位
...
```java
    ...
    final Entry<K,V> getEntry(Object key) {
        ...
        for (Entry<K,V> e = table[indexFor(hash, table.length)]; //得到冲突链表
            ...
            Object k;
            if (e.hash == hash &&
                ((k = e.key) == key || (key != null && key.equals(k)))) //依据equals()方法判断是否相等
                return e;
        }
        return null;
    }
...
![](images/6_Map_HashMap&HashSet/HashMap_3.png)
...
```java
    ...
    void addEntry(int hash, K key, V value, int bucketIndex) {
        if ((size >= threshold) && (null != table[bucketIndex])) {
            ...
            bucketIndex = indexFor(hash, table.length); //hash % table.length
        }
		//在冲突链表头部插入新的entry
...
![](images/6_Map_HashMap&HashSet/HashMap_4.png)
...
例如：容量为2，负载因子为0.75，当插入第2个值时就会进行resize  （2 * 0.75 = 1）。

假设我们要向容量为2的容器中用不同的线程插入A,B,C三个值，在resize之前打个断点，没有resize之前的数据可能如下所示：

![](images/6_Map_HashMap&HashSet/HashMap_6.png)

因为resize之后，会重新计算Hash值，如果使用单链表的**头插法**方式，**同一位置上新元素总会被放到链表的头部位置**，在旧数组中，一条Entry链上的元素，通过计算，可能会被放在新数组不同的位置上。可能会出现如下情况：

![](images/6_Map_HashMap&HashSet/HashMap_7.png)

在多线程情况下，有可能出现循环链表：

![](images/6_Map_HashMap&HashSet/HashMap_8.png)

# Java8 HashMap

JDK: 1.8.0_221

![](images/6_Map_HashMap&HashSet/HashMap_5.png)
